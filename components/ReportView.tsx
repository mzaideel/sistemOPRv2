...
const ReportView: React.FC<ReportViewProps> = ({ data, reportNumber = 1, onBack, onEdit }) => {
  ...
  // Format tarikh ke hb/bln/thn | Hari (Contoh: 21/01/2026 | Rabu)
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fadeIn mb-20 px-4">
      <div className="no-print flex items-center justify-between mb-8 bg-white/80 backdrop-blur-md p-4 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={onEdit} disabled={isGenerating} title="Edit Laporan" className="p-3 bg-amber-500 text-white rounded-2xl hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 active:scale-95 group disabled:opacity-50">
              <ICONS.Edit />
            </button>
            <button onClick={handleDownload} disabled={isGenerating} title="Muat Turun PDF" className="p-3 bg-slate-900 text-white rounded-2xl hover:bg-slate-800 transition-all shadow-lg active:scale-95 group disabled:opacity-50">
              <ICONS.Download />
            </button>
            <button onClick={handlePrint} disabled={isGenerating} title="Cetak Laporan" className="p-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/25 active:scale-95 group disabled:opacity-50">
              <ICONS.Print />
            </button>
          </div>
        </div>
      ...
      <div ref={oprRef} className="opr-page bg-white shadow-2xl border border-slate-100 mx-auto font-sans relative overflow-hidden" style={{ width: '210mm', height: '297mm', padding: '0', minWidth: '210mm', minHeight: '297mm' }}>
          ...
          <div className="col-span-6 bg-slate-50/20 p-6 flex flex-col items-center border-l-2 border-slate-100 overflow-hidden">
            ...
            <div className="flex flex-col gap-2 w-full items-center justify-center">
              {[0, 1, 2, 3].map((i) => {
                ...
                return (
                  <div key={i} className="relative bg-white border border-slate-100 p-0.5 shadow-sm overflow-hidden flex flex-col" style={{ width: '90mm', height: '53.1mm' }}>
                    <div className="w-full h-full overflow-hidden bg-slate-50 relative">
                      {img ? (
                        <img src={img.url} crossOrigin="anonymous" className={`w-full h-full ${img.isLandscape ? 'object-cover' : 'object-contain bg-slate-100'}`} alt={`Foto ${i + 1}`} />
                      ) : (
                      ...
                      {img?.caption && (
                        <div className="absolute bottom-1 left-1 right-1 bg-white/80 backdrop-blur-sm px-1.5 py-0.5 rounded border border-white/40 shadow-sm">
                          <p className="text-[6px] font-black text-slate-800 uppercase tracking-tight text-center leading-tight line-clamp-1">{img.caption}</p>
                        </div>
                      )}
                    </div>
                  </div>
